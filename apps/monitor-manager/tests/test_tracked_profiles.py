...
# apps/monitor-manager/tests -> repo root -> the x11 module's tracked profiles
TRACKED_PROFILES = Path(__file__).resolve().parents[3] / "modules" / "x11" / "bspwm" / "profiles"
...
class TestTrackedProfiles(unittest.TestCase):
    ...
    def test_every_edid_pin_is_a_truncated_hash(self):
        """EDID pins must be 16 lowercase hex chars (monitor_manager/edid.py hash keys);
