...
from .state.desired import DesiredBar, DesiredState
        ...
        for bar in desired.bars:
            b.emit(
                PolybarLaunch(
                    output=bar.output,
                    bar_definition="main",
                    env=bar_env(bar, owns_tray=bar.output in primary_outputs),
                )
            )


# The variables the tracked polybar config (modules/x11/polybar/shades) reads
# with ${env:NAME:fallback}. tests/test_polybar_contract.py holds the two
# sides to this list.
POLYBAR_ENV_VARS = (
    "MONITOR",
    "FONT_0",
...
def bar_env(bar: DesiredBar, owns_tray: bool) -> Tuple[Tuple[str, str], ...]:
    """The environment one polybar instance is launched with."""
    return (
        ("MONITOR", bar.output),
        ("FONT_0", f"JetBrainsMono Nerd Font:pixelsize={bar.font_size};3"),
        ("FONT_1", f"JetBrainsMono Nerd Font:pixelsize={bar.font_size + 2};3"),
        ("MODULES_LEFT", bar.modules_left),
        ("MODULES_CENTER", bar.modules_center),
        ("MODULES_RIGHT", _place_tray(bar.modules_right, owns_tray)),
    )
